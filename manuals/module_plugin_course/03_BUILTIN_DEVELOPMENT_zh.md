...
**与 Plugin 模式的关系**：
- ✅ **可以复用同一套模块代码**：同一个模块往往既可以打成插件，也可以作为 built-in 发布
- 📍 **核心区别不只是位置**：还包括进入壳层的路径、类加载方式和界面呈现方式
  - Plugin 模式：`~/.egps2/config/plugin/`
  - Built-in 模式：`dependency-egps/`
- 🔄 **通常可以互换**：同一个 JAR 文件常常可以在两个位置之间移动，但最终是否显示为当前可用模块仍取决于配置合并结果
...
**重要**：Built-in 模式和 Plugin 模式在模块代码层面很接近，但不要把它们写成“运行时完全一样”。
...
### 需要单独注意：
- ⚠️ Plugin 模式依赖 `eGPS2.plugin.properties` 中的 `launchClass`，并可选使用 `dependentJars`
- ⚠️ Built-in 模式作为类路径模块时，不以 `eGPS2.plugin.properties` 作为主加载前提
- ⚠️ “被发现”不等于“自动激活”
- ⚠️ Module Gallery 跟随当前 active loader set，而不是所有扫描结果
...
| **发布方式** | 用户自行安装 | 随应用一起发布 |
| **配置文件角色** | `eGPS2.plugin.properties` 参与插件入口解析 | 主要依赖类路径与模块发现，不以该文件为主 |
...
1. **模块代码可以相同**：同一套实现通常可以同时支持 Plugin 与 Built-in 两种部署方式
2. **运行时并不完全相同**：位置、类加载路径、入口解析与界面呈现都有差异
3. **可以转换**：很多情况下只需调整部署位置，但最终激活状态仍由当前配置合并结果决定

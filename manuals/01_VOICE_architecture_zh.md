...
- **基础 GUI 层**：`VersatileOpenInputClickAbstractGuiBase`、`InputAreaPanel` 等类提供共用输入区行为和外围 GUI 外壳
- **书签与状态层**：`BookmarkDisplayPanel`、`BookmarkOperationPanel`、`BookMarkNode`、`EditScriptState` 等类负责已保存示例、linked bookmark 行为和编辑状态
- **事件处理层**：`EventUniformlyProcessor` 负责协调各个子面板之间的通用交互流程
- **参数层**：`bean/`、`template/` 下的类负责参数建模、解析、赋值和示例生成
- **Fast VOICE Tab 层**：`fastmodvoice/` 下的类，包括 `TabModuleFaceOfVoice` 和 `DockableTabModuleFaceOfVoice`，负责 Tab 与 dockable 形态的 VOICE 模块
...
### 为什么 Dockable SubTab 要被区别对待

`DockableTabModuleFaceOfVoice` 下的 SubTab 本质上是一个父 VOICE 模块的内部组成部分。
...
这也是模块发现相关文档需要单独为 VOICE SubTab 写排除规则的原因。

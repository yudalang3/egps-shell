...
`eGPS2.plugin.properties` 是 eGPS 插件/模块系统的配置文件，用于描述插件的基本信息和依赖关系。
...
**注意**：
- 如果不提供，系统可能使用类名作为显示名称
- 这个名称主要用于日志记录，不是模块在 UI 中的显示名称
...
| `pluginName` | **推荐** | 影响日志可读性 |
| `version` | **推荐** | 便于版本管理 |
| `author` | 可选 | 仅用于信息记录 |
| `description` | 可选 | 仅用于信息记录 |
...
- ✅ 采用新格式：`key=value`
- ✅ 主要配置项：`launchClass`（必需）
- ✅ 推荐配置项：`pluginName`, `version`, `author`, `description`
- ✅ 可选配置项：`dependentJars`

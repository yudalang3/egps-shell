...
public class NinePatchChunk implements Serializable {
	...
	public void draw(BufferedImage image, Graphics2D graphics2D, int x, int y, int scaledWidth, int scaledHeight,
				...
				while (x < scaledWidth - 1) {
					...
					if (!vStretch) {
						if (hStretch) {
							...
							g.drawImage(image, x, y, x + width, y + r.height, r.x, r.y, r.x + r.width, r.y + r.height,
									null);
							x += width;
						} else {
							r = this.mFixed.get(fixedIndex++);
							g.drawImage(image, x, y, x + r.width, y + r.height, r.x, r.y, r.x + r.width, r.y + r.height,
									null);
							x += r.width;
						}
						height = r.height;
						...
						vExtra = (float) r.height / data.mVerticalPatchesSum;
						height = (int) (vExtra * vRemainder / vWeightSum);
						g.drawImage(image, x, y, x + r.width, y + height, r.x, r.y, r.x + r.width, r.y + r.height,
								null);
						x += r.width;
					}
	...
	private void findPatches(BufferedImage image) {
		...
		this.mFixed = this.getRectangles(left.mFirst, top.mFirst);
		this.mPatches = this.getRectangles(left.mSecond, top.mSecond);
		if (this.mFixed.size() > 0) {
			this.mHorizontalPatches = this.getRectangles(left.mFirst, top.mSecond);
			this.mVerticalPatches = this.getRectangles(left.mSecond, top.mFirst);
		} else if (top.mFirst.size() > 0) {
			this.mHorizontalPatches = new ArrayList<>(0);
			this.mVerticalPatches = this.getVerticalRectangles(height, top.mFirst);
		} else if (left.mFirst.size() > 0) {
			this.mHorizontalPatches = this.getHorizontalRectangles(width, left.mFirst);
		...
		this.mHorizontalPadding = this.getPadding(top.mFirst);
		left = this.getPatches(column, result);
		this.mVerticalPadding = this.getPadding(left.mFirst);
	}
	...
	private Pair<Integer> getPadding(List<Pair<Integer>> pairs) {
		...
		return new Pair<>(pairs.get((int) 0).mSecond - pairs.get((int) 0).mFirst,
				pairs.get((int) index).mSecond - pairs.get((int) index).mFirst);
	}

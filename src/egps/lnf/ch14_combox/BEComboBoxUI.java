...
public class BEComboBoxUI extends BasicComboBoxUI 
	implements egps.lnf.BeautyEyeLNFHelper.__UseParentPaintSurported
{
    /**
     * Creates the ui.
     *
    ...
    public void paintCurrentValueBackground(Graphics g,Rectangle bounds,boolean hasFocus) 
    {
    	...
    	if(!isUseParentPaint())
    	{
    		if ( comboBox.isEnabled() )
    			egps.lnf.ch6_textcoms.__Icon9Factory__.getInstance().getTextFieldBgNormal()
    			.draw((Graphics2D)g, 0,0,comboBox.getWidth(),comboBox.getHeight());
    		else
    			egps.lnf.ch6_textcoms.__Icon9Factory__.getInstance().getTextFieldBgDisabled()
    			.draw((Graphics2D)g, 0,0,comboBox.getWidth(),comboBox.getHeight());
    	}
    	else
    	{

...
//			// rover时图片颜色变成红色
//			btn.setRolloverIcon(BEUtils.filterWithRescaleOp((ImageIcon) ico, 2f, 0f, 0f, 0f));
//			// press时图片颜色变成淡红色
			btn.setPressedIcon(BEUtils.filterWithRescaleOp((ImageIcon) ico, 2f, 0f, 0f, 0.5f));
		}
	}
	 ...
	 * @since 3.5
	 */
	public static ImageIcon filterWithRescaleOp(ImageIcon iconBottom, float redFilter, float greenFilter,
			float blueFilter, float alphaFilter) {
		try {
	...
	/**
	 * 绘制虚线框（本方法适用于对4个边的可选绘制情况下，可能会有4舍5入的小误差 ，除了要可选绘制4个边外，一般不推荐使用）.<br>
	 * .
	 *
	 * @param g      the g
	 * @param x      the x
	...
	/**
	 * 绘制虚线框（本方法适用于对4个边的可选绘制情况下，可能会有4舍5入的小误差 ，除了要可选绘制4个边外，一般不推荐使用）.
	 *
	 * @param g               the g
	 * @param x               the x
	 * @param y               the y
	 ...
	 * @param separator_space 虚线段空白长度 add by js,2009-08-30
	 */
	public static void drawDashedRect(Graphics g, int x, int y, int width, int height, int arcWidth, int arcHeight,
			int separator_solid, int separator_space) {
		BEUtils.setAntiAliasing((Graphics2D) g, true);

		// 虚线样式
		Stroke oldStroke = ((Graphics2D) g).getStroke();
		Stroke sroke = new BasicStroke(1, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0,
				new float[] { separator_solid, separator_space }, 0);// 实线，空白
	 ...
	 */
	public static void drawDashedRect(Graphics g, int x, int y, int width, int height, int step// ,boolean
																								// drawLeft$Right
...
//    	if(drawLeft$Right)
	...
	public static Color getColor(Color basic, int r, int g, int b) {
		return new Color(getColorInt(basic.getRed() + r), getColorInt(basic.getGreen() + g),
				getColorInt(basic.getBlue() + b), getColorInt(basic.getAlpha()));
	}
	...
	public static Color getColor(Color basic, int r, int g, int b, int a) {
		return new Color(getColorInt(basic.getRed() + r), getColorInt(basic.getGreen() + g),
				getColorInt(basic.getBlue() + b), getColorInt(basic.getAlpha() + a));
	}

...
import egps.lnf.utils.WindowTranslucencyHelper;
import egps.lnf.widget.ImageBgPanel;

/**
 * 自定义透明背景的弹出工厂实现类. 本类其实与Swing的UI机制关联不大，完全可以独立于L&F使用。
 * 
 * @author Jack Jiang(jb2011@163.com), 2012-05-18; yudalang, 2024-04-22
 * @version 1.0
 */
...
//						isTooltip ? 1.0f : 0.95f);//0.85f : 0.95f);//0.8f : 0.95f);
				WindowTranslucencyHelper.setOpacity(component, isTooltip ? 1.0f : isComboBoxPopup ? 0.95f : 0.95f);// 0.85f
																													// :
																													// 0.95f);//0.8f
																													// :
																													// 0.95f);
...
//				}
				...
				ImageBgPanel imageContentPane = new ImageBgPanel()
						.setN9(isTooltip ? __Icon9Factory__.getInstance().getTooltipBg() : isComboBoxPopup ? // 是下拉框列表则使一个跟JScrollPane一样的背景图，好看！
								egps.lnf.ch4_scroll.__Icon9Factory__.getInstance().getScrollPaneBorderBg()
				...
				if (contents instanceof JComponent) {
					...
					// ############################################
					// * 关于Java Bug
					// 6683775,地址：http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=6683775，解决于java1.6.0_14
					// *
					// * 【Bug说明】：
...
//					System.out.println("The setDoubleBuffered false " + contents);
					// * ######################################### Bug FIX END
					// ##############################################

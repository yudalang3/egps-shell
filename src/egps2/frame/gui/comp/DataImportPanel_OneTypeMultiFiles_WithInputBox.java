...
/**
 * DataImportPanel_OneTypeMultiFiles_WithInputBox supports the main eGPS window, actions, or tab management.
 */
public class DataImportPanel_OneTypeMultiFiles_WithInputBox extends JPanel {

	private JRadioButton radioButtonImportFiles;
	private JRadioButton radioButtonImportDir;
	...
	private JTextField jTextField_importDir;
	 ...
	 * Create the panel.
	 */
	public DataImportPanel_OneTypeMultiFiles_WithInputBox(Class<?> clz) {
		...
		gridBagLayout.rowHeights = new int[] { 0, 40, 0, 50, 20, 0, 0 };
		...
		gbc_jtextAreaWithString_directContents.insets = new Insets(0, 5, 5, 10);
		...
		gbc_questionLabelImportFiles.insets = new Insets(0, 0, 5, 5);
		...
		jtextAreaWithString_importFiles = new JTextAreaWithDefaultContent_MouseAdapter();
		jtextAreaWithString_importFiles.setTextContents(Arrays.asList("Drag file or directory here."));
		...
		JTextAreaTransferHandler newHandler = new JTextAreaTransferHandler(list -> {
			...
			textAreaOfFileList.setText(stringJoiner.toString());
		});

		textAreaOfFileList.setTransferHandler(newHandler);

		GridBagConstraints gbc_jtextAreaWithString_importFiles = new GridBagConstraints();
		gbc_jtextAreaWithString_importFiles.gridheight = 3;
		gbc_jtextAreaWithString_importFiles.insets = new Insets(0, 5, 5, 10);
				...
				button4LoadFiles.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						...
						if (showOpenDialog == EGPSFileChooser.APPROVE_OPTION) {
							...
							textAreaOfFileList.setText(stringBuilder.toString());
						...
						gbc_btn_loadFiles.insets = new Insets(0, 0, 5, 0);
						gbc_btn_loadFiles.gridx = 3;
						gbc_btn_loadFiles.gridy = 4;
						add(button4LoadFiles, gbc_btn_loadFiles);

		radioButtonImportDir = new JRadioButton("Import directory");
		radioButtonImportDir.setFont(globalFont);
		GridBagConstraints gbc_radioButtonImportDir = new GridBagConstraints();
		gbc_radioButtonImportDir.anchor = GridBagConstraints.WEST;
		gbc_radioButtonImportDir.insets = new Insets(0, 0, 0, 5);
		gbc_radioButtonImportDir.gridx = 0;
		gbc_radioButtonImportDir.gridy = 5;
		add(radioButtonImportDir, gbc_radioButtonImportDir);
		buttonGroup.add(radioButtonImportDir);
		radioButtonImportDir.addItemListener(e -> {
			if (e.getStateChange() == ItemEvent.SELECTED) {
				inputdirConsumer.accept(new File(jTextField_importDir.getText()));
			}
		});

		JLabel questionLabelImportDir = new JLabel("");
		questionLabelImportDir.setToolTipText(
				"If there are more than three files, please set up a directory for those files, and input the directory here.");
		questionLabelImportDir.setIcon(EGPSShellIcons.get("help_blue.png"));
		GridBagConstraints gbc_questionLabelImportDir = new GridBagConstraints();
		gbc_questionLabelImportDir.anchor = GridBagConstraints.EAST;
		gbc_questionLabelImportDir.insets = new Insets(0, 0, 0, 5);
		gbc_questionLabelImportDir.gridx = 1;
		gbc_questionLabelImportDir.gridy = 5;
		add(questionLabelImportDir, gbc_questionLabelImportDir);

		jTextField_importDir = new JTextField();
		jTextField_importDir.setFont(globalFont);
		jTextField_importDir.setColumns(10);
		GridBagConstraints gbc_textFieldDir = new GridBagConstraints();
		gbc_textFieldDir.insets = new Insets(0, 5, 0, 10);
		gbc_textFieldDir.fill = GridBagConstraints.HORIZONTAL;
		gbc_textFieldDir.gridx = 2;
		gbc_textFieldDir.gridy = 5;
		add(jTextField_importDir, gbc_textFieldDir);

		jTextField_importDir.setTransferHandler(new EGPSTextTransferHandler());
		
		JButton btn_loadDirectory = new JButton("Load");
		btn_loadDirectory.setFont(globalFont);
		btn_loadDirectory.setToolTipText("Click to open file chooser.");
		btn_loadDirectory.addActionListener(e -> {
			EGPSFileChooser egpsFileChooser = new EGPSFileChooser(clz);
			egpsFileChooser.setFileSelectionMode(EGPSFileChooser.DIRECTORIES_ONLY);
			int showOpenDialog = egpsFileChooser.showOpenDialog();
			if (showOpenDialog == EGPSFileChooser.APPROVE_OPTION) {
				jTextField_importDir.setText(egpsFileChooser.getSelectedFile().getAbsolutePath());
			}
		});
		
		GridBagConstraints gbc_btnNewButton_loadFile_1 = new GridBagConstraints();
		gbc_btnNewButton_loadFile_1.insets = new Insets(4, 0, 0, 0);
		gbc_btnNewButton_loadFile_1.gridx = 3;
		gbc_btnNewButton_loadFile_1.gridy = 5;
		add(btn_loadDirectory, gbc_btnNewButton_loadFile_1);
	...
	private void addListeners() {
		...
		jTextField_importDir.getDocument().addDocumentListener(new DocumentListener() {

			@Override
			public void removeUpdate(DocumentEvent e) {
				if (radioButtonImportDir.isSelected()) {
					inputdirConsumer.accept(new File(jTextField_importDir.getText()));
				} else {
					radioButtonImportDir.setSelected(true);
				}
			}
	...
	public List<File> getInputFile() {
		...
		if (radioButtonImportFiles.isSelected()) {
			...
			File file = new File(jTextField_importDir.getText());
			if (file.isDirectory()) {
	...
	public void setDialogContent4inputFormat(List<String> strings) {
		questionLabelImportContent.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				SwingDialog.showInformationDialog(new Dimension(800, 600), strings, UnifiedAccessPoint.getInstanceFrame(), "Format statement");
			}
		});
	}

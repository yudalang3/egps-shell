 ...
 * <p>
 * This class is directly copied from the Apache POI project. Changes are:
 * DrawString method is not take the transform info. into account.
 * </p>
 * 
 * <p>
 * 发现无法导出渐变，用位图来填充
...
public class SLGraphics extends Graphics2D implements Cloneable {
	...
	private XMLSlideShow _ppt;

	/**
	 * Construct Java Graphics object which translates graphic calls in ppt drawing
	 * layer.
	 *
	 * @param group The shape group to write the graphics calls into.
	 * @param ppt
	 */
	public SLGraphics(GroupShape<?, ?> group, XMLSlideShow ppt) {
		this._group = group;
		this._ppt = ppt;
	...
	public void drawString(String s, float x, float y) {
		...
		/*
		 * Even if top and bottom margins are set to 0 PowerPoint always sets extra
		 * space between the text and its bounding box.
		 *
		...
		/*
		 * In powerpoint anchor of a shape is its top left corner. Java graphics sets
		 * string coordinates by the baseline of the first character so we need to shift
		 * up by the height of the textbox
		 */
		y -= height / 2 + ascent / 2;
		 ...
		 * In powerpoint anchor of a shape is its top left corner. Java graphics sets
		 * string coordinates by the baseline of the first character so we need to shift
		 * down by the height of the textbox
		 */
		txt.setAnchor(new Rectangle((int) x, (int) y, (int) width, (int) height));

		/*
		 * Add by the angle of the transformation matrix, ydl
		 */
		double rotationAngle = getRotationAngle(_transform);
//        txt.setRotation(60.0);
		txt.setTextRotation(-rotationAngle);
	}

	private double getRotationAngle(AffineTransform transform) {
	...
	public void fill(Shape shape) {
		if (this._paint instanceof LinearGradientPaint) {
			LinearGradientPaint lPaint = (LinearGradientPaint) _paint;
			Rectangle2D bounds2d = shape.getBounds2D();
			// 添加图片到 PowerPoint
			BufferedImage gradientImage = new BufferedImage((int) bounds2d.getWidth(), (int) bounds2d.getHeight(),
					BufferedImage.TYPE_INT_RGB);
			Graphics2D g2d = gradientImage.createGraphics();

			LinearGradientPaint linearGradientPaint = new LinearGradientPaint((float) 0, (float) 0,
					(float) bounds2d.getWidth(), (float) bounds2d.getHeight(), lPaint.getFractions(),
					lPaint.getColors());
			g2d.setPaint(linearGradientPaint);
			g2d.fillRect(0, 0, (int) bounds2d.getWidth(), (int) bounds2d.getHeight());
			g2d.dispose();
			...
			try {
				ImageIO.write(gradientImage, "png", imageOut);
				byte[] imageBytes = imageOut.toByteArray();
				pictureData = _ppt.addPicture(new ByteArrayInputStream(imageBytes), PictureData.PictureType.PNG);
			} catch (IOException e) {
				e.printStackTrace();
			}

			PictureShape<?, ?> picture2 = _group.createPicture(pictureData);
			// 定位图片
			picture2.setAnchor(bounds2d);
	 ...
	 * image observer.
	 * <p>
	 *  scaled version of an image will not necessarily be available immediately
	 ...
	 * changed, then <code>drawImage</code> returns <code>true</code>. Otherwise,
	 * <code>drawImage</code> returns <code>false</code> and as more of the image
	 * becomes available or it is time to draw another frame of animation, the
	 * process that loads the image notifies the specified image observer.
	 *
	 ...
	 * is using.  <code>Graphics</code> object cannot be used after
	 ...
	 * rectangle are at <code>x</code> and <code>x&nbsp;+&nbsp;width</code>. The top
	 * and bottom edges are at <code>y</code> and <code>y&nbsp;+&nbsp;height</code>.
	...
	public FontRenderContext getFontRenderContext() {
		boolean isAntiAliased = RenderingHints.VALUE_TEXT_ANTIALIAS_ON
				.equals(getRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING));
		boolean usesFractionalMetrics = RenderingHints.VALUE_FRACTIONALMETRICS_ON
	 ...
	 * the original <code>Transform</code> Cx. In other words, Cx'(p) = Cx(Tx(p)).
	 ...
	 *
	 * @param img   the image to be rendered. This method does nothing if
	 *              <code>img</code> is null.
	 * @param xform the transformation from image space into user space
	 * @see #_transform
	 * @see #setTransform

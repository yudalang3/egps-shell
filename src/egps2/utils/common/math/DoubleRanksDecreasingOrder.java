 ...
 * @since 2.0
 */
public class DoubleRanksDecreasingOrder implements RankingAlgorithm {
	 ...
	 */
	public DoubleRanksDecreasingOrder() {
		super();
	 ...
	 */
	public DoubleRanksDecreasingOrder(TiesStrategy tiesStrategy) {
		super();
	 ...
	 */
	public DoubleRanksDecreasingOrder(NaNStrategy nanStrategy) {
		super();
	 ...
	 *            TiesStrategy to use
	 */
	public DoubleRanksDecreasingOrder(NaNStrategy nanStrategy, TiesStrategy tiesStrategy) {
		super();
		this.nanStrategy = nanStrategy;
		this.tiesStrategy = tiesStrategy;
		randomData = new RandomDataGenerator();
	 ...
	 */
	public DoubleRanksDecreasingOrder(RandomGenerator randomGenerator) {
		super();
	...
	/**
	 * Create a NaturalRanking with the given NaNStrategy, TiesStrategy.RANDOM and
	 * the given source of random data.
	 *
	 * @param nanStrategy
	 *            NaNStrategy to use
	 * @param randomGenerator
	 *            source of random data
	 */
	public DoubleRanksDecreasingOrder(NaNStrategy nanStrategy, RandomGenerator randomGenerator) {
		super();
		this.nanStrategy = nanStrategy;
		this.tiesStrategy = TiesStrategy.RANDOM;
		randomData = new RandomDataGenerator(randomGenerator);
	...
	/**
	 * Rank <code>data</code> using the natural ordering on Doubles, with NaN values
	 * handled according to <code>nanStrategy</code> and ties resolved using
	 * <code>tiesStrategy.</code>
	 *
	 * @param data
	 *            array to be ranked
	...
	/**
	 * Rewrie for the use of input List!
	 */
	public double[] rank(List<Double> data) {
	
		// Array recording initial positions of data to be ranked
		IntDoublePair[] ranks = new IntDoublePair[data.size()];
		for (int i = 0; i < data.size(); i++) {
			ranks[i] = new IntDoublePair(data.get(i), i);
		}
	...
	public static void main(String[] args) {
		...
		DoubleRanksDecreasingOrder dOrder = new DoubleRanksDecreasingOrder();
		System.out.println(Arrays.toString( dOrder.rank(data) ));
		
		DoubleRanks order = new DoubleRanks();
		System.out.println(Arrays.toString( order.rank(data) ));
		
	}

	/**
	 * Returns an array that is a copy of the input array with IntDoublePairs having
	 * NaN values removed.
	 *
	 * @param ranks
	 *            input array
	 ...
	 * input <code>ranks</code> array is expected to take the same value for all
	 * indices in <code>tiesTrace</code>. The common value is recoded according to
	 * the tiesStrategy. For example, if ranks = <5,8,2,6,2,7,1,2>, tiesTrace =
	 * <2,4,7> and tiesStrategy is MINIMUM, ranks will be unchanged. The same array
	 * and trace with tiesStrategy AVERAGE will come out <5,8,3,6,3,7,1,3>.
	 *
	 * @param ranks
	 *            array of ranks
	 * @param tiesTrace
	 *            list of indices where <code>ranks</code> is constant -- that is,
	 *            for any i and j in TiesTrace, <code> ranks[i] == ranks[j]
	 * </code>
	 */
	...
	private static class IntDoublePair implements Comparable<IntDoublePair> {
		...
		/**
		 * Compare this IntDoublePair to another pair. Only the <strong>values</strong>
		 * are compared.
		 *
		 * @param other
